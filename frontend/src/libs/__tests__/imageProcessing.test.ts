	...
	describe("Direct Image Download Tests", () => {
		testImages.forEach((imageUrl) => {
			it(`should download ${imageUrl.split("/").pop()} successfully`, async () => {
				...
				const hasUTF8Corruption =
					buffer.slice(0, 20).includes(0xef) &&
		...
		testImages.forEach((imageUrl) => {
			it(`should process ${imageUrl.split("/").pop()} through Astro endpoint`, async () => {
				...
				if (!response.ok) {
					...
					expect
						.soft(errorText)
						.not.toContain("Could not process image request");
				} else {
	...
	describe("Error Detection Tests", () => {
		it("should detect corrupted image data", async () => {
			...
			const testCases = [
				...
				{
					name: "UTF-8 corrupted",
					data: Buffer.from([0xef, 0xbf, 0xbd, 0xef, 0xbf, 0xbd]),
				},
			];
			...
			for (const testCase of testCases) {
				try {
					...
					expect.fail(
						`Should have failed for ${testCase.name}, but got buffer of length ${result.length}`,
					);
				} catch (error) {
					expect(error).toBeInstanceOf(Error);
					expect((error as Error).message).toMatch(/input|buffer|format/i);
				}
			}
		});
	});
	...
	describe("Encoding Issues Tests", () => {
		it("should detect UTF-8 replacement characters in image data", () => {
			...
			const corruptedData = Buffer.from(
				"efbfbdefbfbdefbfbdefbfbd0043000606060607",
				"hex",
			);

			const hasUTF8Corruption =
				corruptedData.includes(0xef) &&
			...
			const isValidJPEG =
				corruptedData[0] === 0xff && corruptedData[1] === 0xd8;
			expect(isValidJPEG).toBe(false);
		});

		it("should correctly identify valid JPEG data", () => {
			const validJPEG = Buffer.from(
				"ffd8ffdb004300060606060706070808070a0b0a",
				"hex",
			);
			...
			const hasUTF8Corruption =
				validJPEG.slice(0, 20).includes(0xef) &&
