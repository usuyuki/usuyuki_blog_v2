...
同一記事に対してユーザが投稿した絵文字を初回リアクション日時の昇順で表示する。  
上限件数（例: 最大20種）を超えた場合は古いものを優先し、件数の少ないものを非表示にする。
...
- 絵文字は初回リアクション日時の昇順で表示し、まだ誰もリアクションしていない場合は「最初にリアクションしよう」等のプレースホルダーを表示する
